...
{	
	...
	for (int currentItem=0; currentItem <= itemCount; currentItem++)	//for each row in the cache
	{
		for (int weight=0; weight <= capacity; weight++)	//for each column in the cache
		{
			...
			else if (weights[currentItem-1] > capacity) 	//need -1 to account for the fact that the weights array is 0-based
			...
			//the current item can fit into the knapsack, is the combination WITH the current item worth more than the best combination WITHOUT the current item? Choose the max	
				...
				cache[currentItem][weight]= max(values[currentItem-1] + cache[currentItem-1][weight-weights[currentItem-1]], //value of the current item + best value we can get from the remaining capacity
												cache[currentItem-1][weight]);	//the best value we can get from the capacity WITHOUT including the current item
		}
	}
